# LSTM的简洁实现，实现字符级别的语言模型，数据由随机采样生成
# num_epochs = 500
# num_steps = 35
# lr = 1
# max_norm = 1.0
# 在长度为10000的语料库中训练，ppl=1.390
...
class LSTMModel(nn.Module):
    ...
    def get_init_state(self):
        return (
            torch.zeros((1, self.batch_size, self.hidden_size), dtype=torch.float, device=device),
            torch.zeros((1, self.batch_size, self.hidden_size), dtype=torch.float, device=device)
        )
    
    def forward(self, x, state):  # x:[L, B, V] state(H, C):[1, B, H]
        output, _ = self.lstm(x, state)  # output:[L, B, H]
        return self.output_layer(output.reshape(-1, output.shape[-1]))
